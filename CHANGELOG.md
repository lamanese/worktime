...
- **Wochenplan als PDF speichern**: Button «Als PDF speichern» in der Wochenansicht erzeugt ein A4-Querformat-PDF (Zeile pro Mitarbeiter, Spalte pro Tag, Auftraege mit Uhrzeit, Dauer und «Abruf», Abwesenheiten und Feiertage als Label, Sperrvermerk), legt es unter `<PDF-Archiv>/Dienstplan/KWnn-JJJJ.pdf` im Ordner des Archiv-Benutzers ab (vorhandene Datei wird ersetzt) und laedt es zugleich herunter. Ist kein Archiv konfiguriert oder schlaegt die Ablage fehl, kommt das PDF trotzdem, mit Hinweis. Audit-Eintrag «PDF exportiert».
...
- **Woche in eine beliebige Woche kopieren**: Neben «Woche in naechste Woche kopieren» gibt es «Woche kopieren nach …» mit Kalenderwoche und Jahr, auch ueber den Jahreswechsel (z. B. KW 49/2026 nach KW 13/2027); der Dialog zeigt den Zieldatumsbereich und lehnt nicht existierende Wochen ab.
