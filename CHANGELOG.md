...
- **«Erledigt» fuer Vorlagen ohne feste Tage**: blendet die Vorlage fuer die angezeigte Woche in die Zeile «n erledigt» aus, dort rueckgaengig machbar.
- **«Diese Woche ignorieren»** pro Vorlage: nimmt eine Vorlage mit festen Tagen fuer die angezeigte Woche aus der Pruefung (rueckgaengig machbar, Folgewochen unberuehrt, im Audit-Log). In gesperrten Wochen nicht moeglich.
- **Marker «falscher Tag» auf Auftragskarten**: Liegt ein Auftrag aus einer Vorlage mit festen Tagen an einem anderen Wochentag, zeigt die Karte ein rundes rotes Schild mit weissem Balken; der Tooltip nennt die vorgesehenen Tage (nur fuer Planer).
- **Auftrag per Drag and Drop entfernen**: Eine Karte aus dem Wochenplan auf die Vorlagen-Leiste ziehen loescht sie sofort (Ablagezone mit Papierkorb, Toast, Audit-Log wie beim Loeschen im Formular).
- **Hinweis «n Tage offen»** unter der Kalenderwoche im Wochenplan, auch bei ausgeblendeter Vorlagen-Leiste (Klick blendet die Leiste ein). Beim Sperren einer Woche mit offenen festen Tagen fragt ein Dialog mit der Liste der fehlenden Tage nach; Sperren bleibt moeglich.
- «Woche kopieren nach …» uebernimmt die Herkunft der Auftraege, die Zielwoche gilt damit direkt als verteilt.
...
- **Reihenfolge der Zeilen im Dienstplan pro Mitarbeiter einstellbar** (Feld «Reihenfolge im Dienstplan» im Profil).
- **Wochenplan als PDF ins Archiv**: Button «Als PDF ins Archiv» in der Wochenansicht erzeugt ein A4-Querformat-PDF (Zeile pro Mitarbeiter, Spalte pro Tag, Auftraege mit Uhrzeit, Dauer und «Abruf», Abwesenheiten und Feiertage als Label, Sperrvermerk) und legt es unter `<PDF-Archiv>/Dienstplan/KWnn-JJJJ.pdf` im Ordner des Archiv-Benutzers ab (vorhandene Datei wird ersetzt). Nur Admin und HR-Manager (die Datei landet im Konto des Archiv-Benutzers). Ohne konfiguriertes Archiv oder bei Ablagefehler gibt es eine klare Meldung. Audit-Eintrag «PDF exportiert».
- **Notiz-Hinweis auf Auftragskarten**: Karten mit Notiz zeigen ein kleines Warnschild, der Text bleibt als Tooltip.
- **Wochensperre im Dienstplan («Schluessel»)**: Planer sperren eine Woche per Schluessel-Button im Kopf der Wochenansicht; gesperrte Wochen lassen keine Aenderungen mehr zu (kein Anlegen, Verschieben, Bearbeiten, Loeschen, auch nicht per API oder als Ziel von «Woche kopieren»), Auftraege sind nur noch ansehbar, ein Hinweisbanner nennt Sperrer und Zeitpunkt. Entsperren duerfen nur Admin und HR-Manager. Sperren und Entsperren stehen im Audit-Log. Migration V28 (Tabelle `zw_duty_week_locks`).
- **Vorlagen-Leiste im Dienstplan ausblendbar**: Firmeneinstellung «Vorlagen-Leiste im Dienstplan anzeigen» in der Sektion «Dienstplan-Vorlagen» (Standard an) als Vorgabe, dazu ein Auge-Schalter im Kopf der Wochenansicht, mit dem jeder Planer die Leiste fuer sich ein- oder ausblendet (persoenliche Einstellung, ueberschreibt die Vorgabe). Die Vorlagen bleiben erhalten.
- **Woche in eine beliebige Woche kopieren**: «Woche kopieren nach …» (ersetzt «Woche in naechste Woche kopieren») mit Kalenderwoche und Jahr, vorbelegt mit der Folgewoche, auch ueber den Jahreswechsel (z. B. KW 49/2026 nach KW 13/2027); der Dialog zeigt den Zieldatumsbereich und lehnt nicht existierende Wochen ab.
- **Kalenderwoche gross in der Ecke des Wochenplans** (links ueber der Namensspalte, mit Jahr).
