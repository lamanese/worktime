...
class PdfServiceTest extends TestCase {
    ...
    public function testArchivePathKeepsDotsApostrophesAndSpaces(): void {
        // Zeitwerk deviates from upstream here on purpose: folders written by
        // earlier releases must keep their exact names after an update, so only
        // separators, control characters and ".." runs are touched.
        $this->assertSame(
            'WorkTime/Archiv/2026/Dr. O\'Brien-Meier_Anna Lena',
            $this->archiveFolderPath('Dr. O\'Brien-Meier', 'Anna Lena')
        );
        $this->assertSame(
            'WorkTime/Archiv/2026/Meier _ Lea',
            $this->archiveFolderPath('Meier ', ' Lea')
        );
    }

    public function testArchivePathStripsControlCharacters(): void {
        $this->assertSame(
            'WorkTime/Archiv/2026/Mei_er_Lea',
            $this->archiveFolderPath("Mei\x00er", 'Lea')
        );
        $this->assertSame(
            'WorkTime/Archiv/2026/C:_Users_Lea',
            $this->archiveFolderPath('C:\\Users', 'Lea')
        );
    }
